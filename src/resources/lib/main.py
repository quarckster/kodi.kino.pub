...
def show_pagination(pagination, include_current=False, start_from=0):
    """Add "next page" button"""
    if pagination and (int(pagination["current"]) + 1 <= int(pagination["total"])):
        enumerator = 0 if include_current else 1  # if 0 continue with same api page
        page = int(pagination["current"]) + enumerator
        ...
        url = plugin.routing.add_kwargs_to_url(page=page, start_from=start_from)
...
def items(content_type, heading):
    if heading == "alphabet":
        ...
        if plugin.settings.exclude_anime == "true":
            ...
            show_items(items, content_type)
            show_pagination(pagination, include_current=True, start_from=start_from)
        else:
